...
Now, we are going to use EDA to explore one variable. Go to `Viz` and choose `Histogram` for the type. For X Axis, choose `med_hh_inc`. You will see a distribution of median household income in Seattle. <br>

Now, let's create another `Viz` by clicking `+`. This time, choose `Bar` for the type, and for X Axis, choose `SummarizedOffenseDescription` and for Y Axis, choose `total_crime`. You can usually leave the Y Axis as `Number of Rows`, but in this case we need to use the total crime. Once we are done, we can make the plot pretty by sorting the data by Y Axis and change the orientation from Vertical to Horizontal. 
...
Now, let's explore two variables. For this to happen, we will need to turn off the crime type. So, go back to the third step on your right panel, and click `x` on the green `SummarizedOffenseDescription` button. We will only group the data by `tract`. <br>

For the scatter plot, go to `Viz` and choose `Scatter` for the type. On X Axis, choose `pop` and for Y Axis, choose `total_crime`. We can see the relationship between population density and total crime. <br>
...
Now, we will create anothe `Viz` by clicking `+`. For this to work, we will need to include crime type again for the grouping variable. Go back to the third step on the right panel, and click `+` next to the green button and choose  `SummarizedOffenseDescription`. <br>

Now, select the last step on your right panel, and choose `BoxPlot` for the type. For the X Axis, choose `SummarizedOffenseDescription` and choose `total_crime` for the Y Axis. We can see that the boxplot is created. One problem is that there are too many crime types. <br><br>

Let's limit the crime types to top five crimes. We can do this by clicking `+` and choose `Filter` and select `SummarizedOffenseDescription` for the column and `is in` for the opreator. On the Values, pick `CAR PROWL`, `PROPERTY DAMAGE`, `VEHICLE THEFT`, and `BURGLARY`. Go back to `Viz` and examine the boxplot with only the top five crime types.  
